...
export class OmniLogger {
  ...
  /**
   * Recursively redacts sensitive or large fields like 'contents'.
   */
  private static redact(obj: any): any {
    ...
    for (const key of Object.keys(obj)) {
      // Redact 'contents' field which usually contains the full chat history.
      if (key === 'contents' && Array.isArray(obj[key])) {
        result[key] = `[REDACTED: ${obj[key].length} items]`;
      } else {
